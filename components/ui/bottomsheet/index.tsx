import GorhomBottomSheet, { BottomSheetHandle,
  ...
  BottomSheetView as GorhomBottomSheetView } from '@gorhom/bottom-sheet';
...
const bottomSheetBackdropStyle = tva({ base: 'absolute inset-0 flex-1 touch-none select-none bg-black opacity-0' });

const bottomSheetContentStyle = tva({ base: 'mt-2' });
const bottomSheetTriggerStyle = tva({ base: '' });

const bottomSheetIndicatorStyle = tva({ base: 'py-1 w-full items-center rounded-t-lg ' });

const bottomSheetItemStyle = tva({ base: 'p-3 flex-row items-center rounded-sm w-full disabled:opacity-0.4 web:pointer-events-auto disabled:cursor-not-allowed hover:bg-background-50 active:bg-background-100 focus:bg-background-100 web:focus-visible:bg-background-100' });
...
export const BottomSheetTrigger = ({
  ...
  return (
    <Pressable
      ...
      {...props}
      className={bottomSheetTriggerStyle({ className: className })}>
      {props.children}
    </Pressable>
  );
};
...
export const BottomSheetBackdrop = ({
  ...
  return (
    <GorhomBottomSheetBackdrop
      ...
      // @ts-ignore
      className={bottomSheetBackdropStyle({ className: className })}
...
export const BottomSheetDragIndicator = ({
  ...
  return (
    <BottomSheetHandle
      {...props}
      // @ts-ignore
      className={bottomSheetIndicatorStyle({ className: className })}>
      {children}
    </BottomSheetHandle>
  );
};
